...
function library:Window(Info)
    ...
    function insidewindow:ModuleButton(Info)
        ...
        Info.Settings = Info.Settings or {} -- Table of setting definitions

        local moduleEnabled = false
        local moduleKeybind = Enum.KeyCode.LeftAlt

        -- Create the main module button
        ...
        moduleFixLine.Name = "ModuleFixLine"
        moduleFixLine.AnchorPoint = Vector2.new(0.5, 1)
        moduleFixLine.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
        moduleFixLine.BorderSizePixel = 0
        moduleFixLine.Position = UDim2.fromScale(0.5, 0.0526)
        moduleFixLine.Size = UDim2.fromOffset(225, 4)
        moduleFixLine.Parent = moduleButton

        -- Module text (left-aligned, smaller size)
        ...
        moduleText.TextSize = 14  -- Slightly smaller
        ...
        expandArrow.Image = "rbxassetid://7733717447" -- Down arrow
        ...
        expandArrow.Parent = moduleButton

        -- Button overlays for click and right-click
        ...
        -- Settings container (initially hidden, placed below module button)
        local settingsContainer = Instance.new("Frame")
        settingsContainer.Name = "SettingsContainer"
        settingsContainer.BackgroundColor3 = Color3.fromRGB(44, 44, 44)
        settingsContainer.BorderSizePixel = 0
        settingsContainer.ClipsDescendants = true
        settingsContainer.Position = UDim2.new(0, 0, 1, 0)
        ...
        settingsUIList.Padding = UDim.new(0, 2)
        ...
        local function updateContainerSize()
            ...
            -- Update the main frame size to accommodate the expanded settings
            -- We need to adjust the itemContainer size, but that's managed by childAdded/removed.
            -- Since settingsContainer is a child of moduleButton, we need to adjust moduleButton's height.
            -- But moduleButton's height should be fixed; instead, we can adjust the moduleButton's Size when expanded.
            -- However, itemContainer's layout will treat moduleButton as one element. To avoid breaking layout, we could keep moduleButton fixed and place settingsContainer as a separate sibling after moduleButton.
            -- Better: Place settingsContainer as a separate element in itemContainer, below moduleButton.
            -- We'll restructure: moduleButton and settingsContainer are separate siblings in itemContainer.
            -- Let's redesign: we'll create a group frame that holds both the button and the settings.
            -- Since we already created moduleButton as the parent, we need to move settingsContainer to be a sibling.
            -- We'll adjust: we'll create a wrapper frame that contains both moduleButton and settingsContainer.
            -- But to keep changes minimal, we can set moduleButton's Size to include both when expanded.
            -- However, itemContainer uses UIListLayout which sizes children based on their Size. If we change moduleButton's Size, it will affect layout.
            -- We could set moduleButton's Size to include the settings height when expanded, and use ClipsDescendants = false to show the settings.
            -- Let's do that: moduleButton will be the container, with ClipsDescendants = false, and we'll adjust its Size when expanded.
            -- But we need to ensure the background and corners look right.
            -- We'll keep moduleButton's ClipsDescendants = false, and when expanded, increase its Size.Y by settingsHeight.
            -- Also, we need to reposition the settingsContainer to start at the bottom of the module text.
            -- This is getting complex. Simpler: use a separate frame in itemContainer for the settings, and when expanded, insert it after the moduleButton.
            -- We'll do that: we'll create a separate frame `settingsHolder` that is placed in itemContainer after moduleButton, and we'll manage its visibility and height.
            -- To ensure proper ordering, we'll set LayoutOrder of settingsHolder to be moduleButton.LayoutOrder + 0.5 (or use a custom approach).
            -- We'll use a Frame with BackgroundColor3 = Color3.fromRGB(44,44,44) and corner radius.
            -- Let's implement that.
        end

        -- We'll restructure: we'll create a settingsHolder as a separate sibling frame in itemContainer.
        -- We'll manage its position manually or by using a UIListLayout with LayoutOrder.
        -- We'll set the settingsHolder's LayoutOrder to be moduleButton.LayoutOrder + 0.5 (floating).
        -- But UIListLayout uses integer order, so we can set it to moduleButton.LayoutOrder + 1 and reorder when needed.
        -- We'll insert the settingsHolder after moduleButton.

        -- To avoid complexity, we can make the settingsHolder a child of moduleButton, but we'll set moduleButton's ClipsDescendants = false and adjust its size.
        -- We'll do that: moduleButton will have ClipsDescendants = false, and when expanded, its Size.Y will increase by settingsHeight.
        -- Then we need to position settingsContainer correctly.
        -- We'll set settingsContainer.Position = UDim2.new(0, 0, 1, 0) relative to moduleButton, so it appears below the button part.
        -- We'll also add a bottom padding to moduleButton to make space.
        -- This works as long as moduleButton's background covers the whole area.
        -- We'll also add corner radius to both the top and bottom when expanded.

        -- We'll redesign: moduleButton is the main container, ClipsDescendants = false.
        -- It contains the button part (a frame) and the settingsContainer.
        -- The button part is a frame that holds the text and arrow, with a fixed height of 38.
        -- The settingsContainer is below it.
        -- When expanded, we set moduleButton.Size = UDim2.fromOffset(225, 38 + settingsHeight).
        -- The button part stays at top.

        -- Let's rebuild moduleButton:

        -- The main moduleButton frame (this will be the container)
        -- We'll destroy the previous moduleButton and rebuild.

        -- But we've already created moduleButton. We'll repurpose it.

        -- We'll make moduleButton ClipsDescendants = false.
        moduleButton.ClipsDescendants = false

        -- Create a inner button frame that holds text and arrow (fixed height)
        ...
        -- Move the corner to the buttonFrame? But we want the whole moduleButton to have corners.
        -- We'll apply UICorner to moduleButton, and also to buttonFrame? Better: moduleButton has UICorner, and buttonFrame has transparent background? Actually we want buttonFrame to have the same color as moduleButton.
        -- We can set moduleButton.BackgroundColor3 = Color3.fromRGB(36,36,36) and no background for buttonFrame, but then when expanded, the bottom part will have same color.
        -- We'll keep moduleButton's background and remove buttonFrame's background.
        -- We'll just move the existing children (moduleText, expandArrow, clickButton) into buttonFrame.
        ...
        -- Add a small divider line between button and settings when expanded
        ...
        -- Move settingsContainer to be a child of moduleButton, but with Position = UDim2.new(0,0,1,0) relative to buttonFrame? Actually we want it below buttonFrame.
        -- We'll set settingsContainer.Position = UDim2.new(0, 0, 1, 0) (relative to moduleButton).
        -- But moduleButton's height will change, so we need to anchor settingsContainer to the bottom of buttonFrame.
        -- Better: make buttonFrame fixed height 38, and settingsContainer placed below it.
        -- We'll set settingsContainer.Position = UDim2.new(0, 0, 1, 0) -- this places it just below the buttonFrame (since buttonFrame is at top).
        -- We'll ensure buttonFrame has a fixed size.

        -- Also we need to adjust moduleButton's size when expanded.
        -- We'll store the current settingsHeight and update.

        -- We'll remove the previous settingsContainer and recreate.
        ...
        settingsContainer.BorderSizePixel = 0
        settingsContainer.ClipsDescendants = true
        settingsContainer.Position = UDim2.new(0, 0, 1, 0) -- below buttonFrame
        settingsContainer.Size = UDim2.new(1, 0, 0, 0)
        settingsContainer.Visible = false
        settingsContainer.ZIndex = 1
        settingsContainer.Parent = moduleButton
        ...
        -- We'll also add corner radius to settingsContainer to match moduleButton.
        local settingsCorner = Instance.new("UICorner")
        settingsCorner.CornerRadius = UDim.new(0, 4)
        settingsCorner.Parent = settingsContainer

        -- We'll need a function to add settings controls (Toggle, Slider, etc.) to settingsContainer.
        -- We'll create helper functions inside the module.

        -- We'll also automatically add a Keybind control at the end.
        local function addKeybindControl()
            -- Create a Keybind control similar to the one in the main library.
            local keybindInfo = {
                Text = "Keybind",
                Default = moduleKeybind,
                Callback = function()
                    -- Toggle the module when keybind is pressed
            ...
            -- We'll use the library's standard Keybind creation but with smaller text size and adapted to settingsContainer.
            -- We'll replicate the Keybind creation code from insidewindow:Keybind but with modifications.
            -- Since we're inside ModuleButton, we can define a local function that creates a keybind control.
            -- To avoid code duplication, we'll call a function that creates a keybind control in the settingsContainer.
            -- We'll create a function `createSettingsKeybind` that takes the container and info.
            ...
            -- Add the keybind control to settingsContainer
            createSettingsKeybind(settingsContainer, keybindInfo)
            settingsHeight = settingsHeight + 24
        end

        -- Function to add a settings control (generic)
        local function addSettingControl(settingType, settingInfo)
            -- We'll use the existing components from insidewindow, but we need to create them in settingsContainer.
            -- We can reuse the code from insidewindow:Toggle, Slider, etc. but with modified sizes and parent.
            -- Since we're inside ModuleButton, we can define local functions that mimic those.
            -- However, to keep code clean, we can create a simple API for adding controls.
            -- We'll implement each control type.
            -- For simplicity, we'll just replicate the code from the main library with adjustments.
            -- We'll keep the same structure but change parent and sizes.
            -- We'll also ensure they use smaller text size (11-12).
            -- We'll implement Toggle, Slider, Keybind, Button, Label, Dropdown.
            ...
            elseif settingType == "Keybind" then
                -- Already handled separately; we can skip if we want to avoid duplicates
                -- But we can allow custom keybinds in settings, we'll add a generic keybind control.
                -- We'll reuse the createSettingsKeybind function.
            ...
            elseif settingType == "Button" then
                ...
                buttonText.Text = settingInfo.Text or "Button"
                buttonText.TextColor3 = Color3.fromRGB(214, 214, 214)
        ...
        -- Add all settings from Info.Settings
        for _, setting in ipairs(Info.Settings or {}) do
            local settingType = setting.Type or "Toggle"
            addSettingControl(settingType, setting)
        end

        -- Automatically add Keybind control at the bottom
        addKeybindControl()

        -- Now we have settingsHeight calculated. We'll store it and update the container size when expanded.
        local function updateContainerSize()
            settingsContainer.Size = UDim2.new(1, 0, 0, settingsHeight)
        end
        updateContainerSize()
        ...
        local function toggleExpand()
            ...
            -- Update backgroundFrame size (itemContainer's size changes automatically because moduleButton size changed)
            -- But itemContainer's size is updated via ChildAdded/Removed, but we changed Size directly, so we need to trigger update.
            -- We'll force a refresh by firing ChildAdded/Removed? Alternatively, we can manually update itemContainer's size.
            -- Since moduleButton is a child of itemContainer, changing its size should automatically update itemContainer's size if it's using a UIListLayout.
            -- However, itemContainer's Size is set manually in ChildAdded/Removed. We'll need to recalculate.
            -- We can manually call a function to update itemContainer size.
            -- For simplicity, we'll just fire a dummy event or recalculate.
            -- We'll create a function inside insidewindow that updates the total size.
            -- Since we have access to itemContainer and BackgroundSize, we can update it.
            -- But BackgroundSize is a local variable in insidewindow. We can't easily update from here.
            -- We'll instead rely on the fact that moduleButton's Size change will trigger a re-layout? The UIListLayout will handle it.
            -- However, itemContainer's Size is set explicitly in ChildAdded/Removed. We need to update it.
            -- We'll add a manual update: we can call itemContainer:GetPropertyChangedSignal("Size")? Not needed.
            -- We'll just set itemContainer.Size to the sum of children heights.
            -- We'll do that in a function.
            -- We'll create a function inside insidewindow that recalculates the itemContainer size.
            -- But we can't easily access insidewindow from here.
            -- We'll instead modify the moduleButton's parent (itemContainer) and update its size.
            -- We'll do: local parent = moduleButton.Parent; if parent then ... but parent is itemContainer.
            -- We'll calculate total height of children.
            local totalHeight = 0
            ...
            BackgroundSize = totalHeight -- update local variable? Not needed.
        end

        -- Click on arrow to toggle expansion
        expandArrow.MouseButton1Click:Connect(toggleExpand)

        -- Right-click on module toggles expansion
        clickButton.MouseButton2Click:Connect(toggleExpand)

        -- Left-click toggles the module (callback)
        clickButton.MouseButton1Click:Connect(function()
            moduleEnabled = not moduleEnabled
            pcall(Info.Callback, moduleEnabled)
            -- Update visual state
    ...
    -- ============================================================
    -- OTHER STANDARD ELEMENTS (Toggle, Slider, Keybind, Dropdown, Label, Prompt)
    -- These remain unchanged from original (they can be used in the main window or in module settings if needed)
    -- ============================================================
    ...
    windowText.TextSize = 16  -- Increased tab name size
