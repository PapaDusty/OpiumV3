...
local library = {}
library.Flags = {}
library.DefaultColor = Color3.fromRGB(5, 133, 104)
...
-- Allow all keys for binding (no blacklist)
local Blacklist = {}
...
function library:Window(Info)
    ...
    -- TabArea with extra padding for rounded corners
    ...
    -- FixLine to cover the rounded top corners of TabArea
    local tabFixLine = Instance.new("Frame")
    tabFixLine.Name = "FixLine"
    tabFixLine.AnchorPoint = Vector2.new(0.5, 0)
    tabFixLine.BackgroundColor3 = Color3.fromRGB(29, 29, 29)
    tabFixLine.BorderSizePixel = 0
    tabFixLine.Position = UDim2.fromScale(0.5, 0)
    tabFixLine.Size = UDim2.fromOffset(windowWidth, 2)
    tabFixLine.Parent = tabArea
    tabFixLine.ZIndex = 2

    -- UIPadding to ensure content doesn't touch the rounded corners
    ...
    -- ModuleArea as ScrollingFrame with max height 550
    ...
    moduleArea.Size = UDim2.fromOffset(windowWidth - 8, 0)
    moduleArea.CanvasSize = UDim2.new(0, windowWidth - 8, 0, 0)
    moduleArea.ScrollBarThickness = 4
    moduleArea.ScrollBarImageTransparency = 0.6
    moduleArea.ClipsDescendants = true
    ...
    moduleList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        local contentHeight = moduleList.AbsoluteContentSize.Y
        moduleArea.CanvasSize = UDim2.new(0, windowWidth - 8, 0, contentHeight)
        local newHeight = math.min(contentHeight, 550)
        moduleArea.Size = UDim2.new(0, windowWidth - 8, 0, newHeight)
        tabArea.Size = UDim2.new(0, windowWidth, 0, newHeight + 8)  -- 8 extra for padding
        BackgroundSize = newHeight + 8
    end)

    -- Keep track of expanded module to collapse others
    ...
    function insidewindow:Module(Info)
        ...
        moduleContainer.Size = UDim2.fromOffset(windowWidth - 8, 38)
        ...
        -- FixLine at the TOP of the module
        local moduleFixLine = Instance.new("Frame")
        moduleFixLine.Name = "FixLine"
        moduleFixLine.AnchorPoint = Vector2.new(0.5, 0)
        moduleFixLine.BackgroundColor3 = Color3.fromRGB(29, 29, 29)
        moduleFixLine.BorderSizePixel = 0
        moduleFixLine.Position = UDim2.fromScale(0.5, 0)
        moduleFixLine.Size = UDim2.fromOffset(windowWidth - 8, 1)
        ...
        local function createKeybindControl(parent, info, onBind)
            ...
            button.MouseButton1Click:Connect(function()
                ...
                KeybindConnection = UserInputService.InputBegan:Connect(function(Key, gameProcessed)
                    if Key.KeyCode ~= Enum.KeyCode.Unknown and not gameProcessed then
        ...
        function moduleAPI:CreateDropdown(info)
            -- Container that holds both header and options, with outline
            local container = Instance.new("Frame")
            container.Name = "DropdownContainer"
            container.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
            container.BackgroundTransparency = 1
            container.Size = UDim2.new(1, 0, 0, 28)
            container.Parent = moduleOptions
            ...
            dropdownOptions.ZIndex = 9999
            dropdownOptions.Parent = container
            ...
            local function UpdateDropdownSize()
                ...
                container.Size = UDim2.new(1, 0, 0, 28 + DropdownSize)
            end
        ...
        function moduleAPI:CreateMultiDropdown(info)
            ...
            multiddOptions.ZIndex = 9999
            multiddOptions.Parent = container
            ...
            local function UpdateMultiDropdownSize()
                ...
                container.Size = UDim2.new(1, 0, 0, 28 + MultiDropdownSize)
            end
        ...
        -- Add default keybind LAST (after all user components)
        ...
        -- Defer addition so it runs after user creates their components
        task.defer(addDefaultKeybind)
        ...
        local function toggleExpand()
            if expandedModule and expandedModule ~= moduleContainer then
                ...
                if prevOptions then
                    ...
                    prev.Size = UDim2.new(0, windowWidth - 8, 0, 38)
                end
            ...
            if expanded then
                ...
                moduleContainer.Size = UDim2.new(0, windowWidth - 8, 0, 38 + optionsHeight + 4)
            else
                expandedModule = nil
                moduleContainer.Size = UDim2.new(0, windowWidth - 8, 0, 38)
            end
            -- Update CanvasSize via the ListLayout event
            moduleList:GetPropertyChangedSignal("AbsoluteContentSize"):Fire()
        end
    ...
    function insidewindow:Button(Info)
        ...
        button.Size = UDim2.fromOffset(windowWidth - 8, 38)
        ...
        fixLine1.Position = UDim2.fromScale(0.5, 1)
        fixLine1.Size = UDim2.fromOffset(windowWidth - 8, 1)
        ...
        buttonTextButton.Size = UDim2.fromOffset(windowWidth - 8, 38)
        ...
        buttonTextLabel.Size = UDim2.fromOffset(windowWidth - 18, 38)
    ...
    local function toggleTab()
        ...
        if WindowOpened.Value then
            ...
            local newHeight = math.min(contentHeight, 550)
            moduleArea.Size = UDim2.new(0, windowWidth - 8, 0, newHeight)
            tabArea.Size = UDim2.new(0, windowWidth, 0, newHeight + 8)
        else
            moduleArea.Size = UDim2.new(0, windowWidth - 8, 0, 0)
            tabArea.Size = UDim2.new(0, windowWidth, 0, 0)
        end
